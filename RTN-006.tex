...
For example, ensuring the documentation infrastructure can accept contributions from the science community (\S~\ref{ssec:mod_docs}); using a publicly-accessible online forum with groups and categories for the Science Collaborations (\S~\ref{ssec:mod_support}); attending Science Collaboration meetings (\S~\ref{ssec:mod_interface}); soliciting Science Collaboration expertise in the issue resolution process (\S~\ref{ssec:mod_coord}); seeking Science Collaboration representation on RSP user committees (\S~\ref{ssec:mod_uc} and \ref{ssec:mod_rac}); and facilitating the Science Collaboration Liaison Program (\S~\ref{ssec:mod_liaisons}).
...
\begin{itemize}
\item coordination with the Users Committee (\S~\ref{ssec:mod_uc}) to include RSP users from the community in UAT campaigns
...
Members will be data rights holders who use the RSP.
...
\begin{itemize}
\item monitoring use of the RSP aspects to understand RSP user needs
\item establishing targets and collecting diversity data about RSP users
\item reviewing Rubin-related publications to understand RSP users' scientific progress
