...
The System Performance department is responsible for ensuring that the LSST is proceeding with the efficiency and fidelity needed to achieve its 10-year science goals -- at minimum, those described in the Science Requirements Document \citedsp{lpm-17} -- and to lead a cutting edge in emergent science.
...
\item \textbf{Data rights holders} are individuals with the right to access, analyze, and publish work based on the Rubin Observatory proprietary data products and services (such as the Rubin Science Platform \citedsp{lse-319}) as described in \citedsp{rdo-013}. The term \textbf{LSST user} or just \textbf{user} represents a data rights holder using the Rubin Observatory data products or services.
\end{itemize}
...
The CET will interface with other Rubin departments to ensure documentation is complete and kept up to date.
...
Other web-based platforms for communication, such as Slack, will continue to be used by the CET for \textit{real-time} discussions with Rubin staff; with Rubin community members participating in virtual or face-to-face meetings and workshops such as the Project and Community Workshop; and with the Science Collaborations (if that remains one of their preferred methods for communication).
...
It is recognized that modes for private Q\&A and issue reporting regarding sensitive scientific topics, or proprietary data, will also need to be supported.
...
In the spirit of the engagement strategies in \S~\ref{ssec:intro_strat}, the CET will develop policies to ensure that diversity, equity, and inclusion are maintained for the limited resource of face-to-face interactions. 
...
The CET will set up the Users Committee's communications infrastructure and meetings, will organize their participation in UAT \S~\ref{ssec:mod_uat}, will ensure that their biannual reports are delivered to the Director, will generate internal Jira tickets to, e.g., resolve issues or request new features, as needed (\S~\ref{sssec:mod_coord_res}). 
...
Early initiatives and strategies related to some of these components are being tested in conjunction with Data Preview 0 \citedsp{rtn-004}.
